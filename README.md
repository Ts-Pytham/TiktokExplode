...
- **Search videos by keyword** — streams results lazily via `IAsyncEnumerable<Video>`
...
`TiktokSearchClient` searches TikTok by keyword and streams results as `Video` objects using `IAsyncEnumerable<Video>`. It **requires Playwright** because TikTok's search API signs requests with dynamic tokens that can only be generated by a real browser session.

```csharp
await using var client = TiktokSearchClient.CreateWithBrowser();

await foreach (var video in client.SearchAsync("funny cats"))
{
    Console.WriteLine($"{video.Author.UniqueId}: {video.Description}");

    // You can also download directly from ISearchClient
...
| Method                                               | Returns                   | Description                                                           |
| ---------------------------------------------------- | ------------------------- | --------------------------------------------------------------------- |
| `SearchAsync(string keyword, CancellationToken)`     | `IAsyncEnumerable<Video>` | Streams video results for the given keyword, paginating automatically |
| `DownloadAsync(Video, CancellationToken)`            | `StreamInfo`              | Downloads video without watermark                                     |
| `DownloadWatermarkedAsync(Video, CancellationToken)` | `StreamInfo`              | Downloads video with watermark                                        |
